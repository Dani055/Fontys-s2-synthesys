...
namespace BusinessLayer.validators
{
    public class MatchValidator
    {
        public int DetermineWinner(TourneyMatch match)
        {
            ...
            {
                throw new ArgumentException("Cannot enter result. The players for this game are not yet decided!");
            }
            else if (match.Player1Points > 30 || match.Player2Points > 30)
            {
                throw new ArgumentException("A player cannot have more than 30 points!");
            }
            else if (match.Player1Points <= 20 && match.Player2Points <= 20)
            {
                throw new ArgumentException("A player needs at least 21 points to win!");
            }
            else if(Math.Abs(match.Player1Points - match.Player2Points) < 2 && (match.Player1Points <= 28 || match.Player2Points <= 28))
            {                                      
               throw new ArgumentException("A player needs to lead by at least 2 points to win!");
            }
            else if (match.Player1Points == 29 && match.Player2Points == 29)
            {
                throw new ArgumentException("At 29-29, the side scoring the 30th point wins the game");
            }
